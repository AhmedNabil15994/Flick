...
trait ControllerSetterAndGetter
{
    ...
    protected function setBasePermissions($model)
    {
        $this->basePermissions =  $this->basePermissions ?? $model->getTable();
    }
    ...
    protected function setViewPath($view = null)
    {
        if ($view) {
            ...
            $this->view_path = $module_name . '::dashboard.' . Str::plural($view_name);
        }
    }
    ...
    protected function setRequest($request = null)
    {
        if ($request) {
            ...
            if (class_exists($class)) {
                ...
                $this->request = 'Modules\Core\Http\Requests\Dashboard\DefaultRequest';
            }
        }
    }
    ...
    {
        if ($resource) {
            $this->model_resource =  is_string($resource) ? $resource : get_class($resource);
        } else {
            ...
            if (class_exists($class)) {
                ...
                $this->model_resource = 'Modules\Core\Transformers\Dashboard\DefaultResource';
            }
        }
    }
    ...
    private function buildNameSpace($folder, $file_tail, $dashboard_folder = true)
    {
        ...
        if (!$dashboard_folder) {
            $class = str_replace('Dashboard\\', '', $class);
        }
    ...
    {
        if (!property_exists($this, $property) && in_array($property, ["model", "repository"])) {
            return null;
        }
    }
}
