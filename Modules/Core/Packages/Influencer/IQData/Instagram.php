...
use Modules\Influencer\Entities\Instagram as InstagramModel;
use Modules\Core\Packages\Influencer\Contracts\InstagramStatisticInterface;

class Instagram implements InstagramStatisticInterface
{
    use IQDataTrait;

    public function setModel(InstagramModel $model)
    {
        $this->model = $model;
        return $this;
    }
    ...
    {
        return 'iqData/'.$this->model->influencer_id."/instagram" . "/".$this->model->id . ".json";
    }
    ...
    private function getDataForModelFromResponse()
    {
        return [
            ...
            "is_business"       => $this->getFromResponse("user_profile.is_business") ,
            "is_hidden"         => $this->getFromResponse("user_profile.is_hidden") ,
            "account_type"      => $this->getFromResponse("user_profile.account_type") ,
            "user_name"         => $this->getFromResponse("user_profile.username") ,
            "account_id"        => $this->getFromResponse("user_profile.user_id") ,
            "followers"       => $this->getFromResponse("user_profile.followers") ,
            ...
            "audience_credibility"  => $this->getFromResponse("audience_followers.data.audience_credibility", 0),
            ...
            "avg_reels_plays"       => $this->getFromResponse("user_profile.avg_reels_plays", 0) ,
            ...
            "audience_types"          => $this->getFromResponse("audience_followers.data.audience_types"),
