...
class YoutubeRepository extends CrudRepository
{
    /**
    ...
    */
    ...
    public function updateStatistic($model, $type="api")
    {
        ...
        $stat = \Modules\Core\Packages\Influencer\InfluencerStatistic::youtube()
                    ->setModel($model);
    ...
    public function inDataTableSearch(&$query, $request)
    {
        if ($request->event_invitation_id) {
            $query->whereNotExists(function ($query) use ($request) {
                ...
                ->whereRaw('youtubes.influencer_id = invitations.influencer_id')
